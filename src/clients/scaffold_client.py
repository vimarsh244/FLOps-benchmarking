...
class ScaffoldClient(NumPyClient):
    ...
    def _unpack_scaffold_params(
        ...
        if is_first_round or num_trainable == 0:
            ...
            server_control = [
                np.zeros_like(p.detach().cpu().numpy(), dtype=np.float32)
    ...
    def fit(
        ...
        full_model, server_control, client_control = self._unpack_scaffold_params(
            parameters, config
        )
        ...
        server_control_tensors = [
            torch.tensor(c, dtype=torch.float32, device=self.device) for c in server_control
        ]
        client_control_tensors = [
            torch.tensor(c, dtype=torch.float32, device=self.device) for c in client_control
        ]
        ...
        for i, (init_p, new_p, c_i, c) in enumerate(
            zip(
                initial_trainable,
            ...
            for batch in self.trainloader:
                ...
                with torch.no_grad():
                    for param, c_i, c in zip(
                        self.model.parameters(), client_control, server_control
                    ):
