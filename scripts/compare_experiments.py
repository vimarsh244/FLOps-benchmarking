...
def compare_accuracy(
    ...
    for i, exp in enumerate(experiments):
        ...
        ax.plot(r, a, "o-", color=color, label=label, linewidth=2, markersize=4, alpha=0.8)
...
def compare_loss(
    ...
    for i, exp in enumerate(experiments):
        ...
        ax.plot(r, l, "o-", color=color, label=label, linewidth=2, markersize=4, alpha=0.8)
...
def compare_convergence(
    ...
    bars1 = ax.bar(
        ...
        width,
        label=f"Rounds to {target_accuracy*100:.0f}% acc",
        color=COLORS[0],
        alpha=0.8,
    )
    ...
    ax2 = ax.twinx()
    bars2 = ax2.bar(
        x + width / 2, final_accuracies, width, label="Final Accuracy", color=COLORS[1], alpha=0.8
    )
...
def generate_comparison_report(
    ...
    compare_convergence(
        experiments, output_path=output_dir / "convergence_comparison.png", show=show
    )
    plt.close()
...
def main():
    ...
    parser.add_argument("experiments", nargs="+", type=str, help="Paths to experiment directories")
    parser.add_argument(
        "-o",
        "--output",
        type=str,
