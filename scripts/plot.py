...
def plot_training_progress(
    ...
    if loss_data:
        ...
        ax1.plot(loss_rounds, loss_values, "o-", color=color_loss, 
                 label="Loss", linewidth=2, markersize=4)
    ...
    if acc_data:
        ...
        ax2.plot(acc_rounds, acc_values, "s-", color=color_acc,
...
def plot_client_participation(
    ...
    # plot heatmap
    cmap = plt.cm.RdYlGn
    im = ax.imshow(participation, aspect="auto", cmap=cmap, 
...
def plot_power_consumption(
    ...
    ax.plot(timestamps, total_power, color="#e74c3c", 
            linewidth=2, label="Total Power")
    ax.fill_between(timestamps, total_power, alpha=0.3, color="#e74c3c")
    
    if any(gpu_power):
        ax.plot(timestamps, gpu_power, color="#3498db",
                linewidth=2, label="GPU Power")
    ...
    if total_power:
        ...
        ax.annotate(f"Total Energy: {total_energy:.2f} Wh",
